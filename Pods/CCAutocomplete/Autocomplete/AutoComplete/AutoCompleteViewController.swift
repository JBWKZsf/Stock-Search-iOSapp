    ...
    @objc func textDidChange(textField: UITextField) {
        ...
        if let numberOfCharacters = numberOfCharacters {
            if numberOfCharacters > self.autocompleteThreshold! {
                ...
                let dataRequest = AsycAutoCompleteDataRequest(symbol: searchTerm)
                dataRequest.getData({data,error-> Void in
                    if(data != nil){
                        var companyOptions: [AutocompletableOption] = []
                        var options:Array<String>=[]
                        for anItem in data as! [Dictionary<String, AnyObject>] {
                            let Symbol = anItem["Symbol"] as! String
                            let Name = anItem["Name"] as! String
                            let Exchange = anItem["Exchange"] as! String
                            let anOption = Symbol+"-"+Name+"-"+Exchange
                            
                           companyOptions.append(AutocompleteCellData(text: anOption, image: UIImage()))
                            
                         }
                        

                        self.autocompleteItems = companyOptions
